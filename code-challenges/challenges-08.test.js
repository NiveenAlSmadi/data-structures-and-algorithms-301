...
function returnTen(str){
  // Solution code here...
  let array = str.split('');
  let N= array.splice(array.length-10);
  return N;
}
...
const containsAnd = (arr) => {
  // Solution code here...
  let and = arr.filter(item=> item.includes('and'));
  return and;
};
...
const oddValues = (arr) => {
  // Solution code here...
  let oddNumb = arr.filter((n)=> !(n%2 === 0));
  return oddNumb;
};
...
const filterStringsWithVowels = (arr) => {
  // Solution code here...
  let vowel = arr.filter(item => item.match(/[aeiou]/));
  return vowel;
};
